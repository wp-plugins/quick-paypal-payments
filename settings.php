...
case 'recurring' : qpp_recurring_codes($id); break;
		}
	echo '</div>';
	}
...
function qpp_form_options($id) {
	...
	if( isset( $_POST['qpp_submit'])) {
		$options = array('title',
                         ...
                         'fixedreference',
                         'fixedamount',
                         'useterms','useblurb','extrablurb','userecurring','recurringblurb','recurring','Dvalue','Wvalue','Mvalue','Yvalue','termsblurb','termsurl','termspage','quantitymax','quantitymaxblurb');
	...
	$content .= '<form action="" method="POST">
		...
		foreach (explode( ',',$qpp['sort']) as $name) {
			switch ( $name ) {
                ...
                $checked = 'checked';
                $options = '<input type="checkbox" style="margin:0; padding: 0; border: none" name="fixedreference" ' . $qpp['fixedreference'] . ' value="checked" /> Display as a pre-set reference<br><span class="description">Use commas to seperate options</span>.';
                break;
				case 'field2': 
...
function qpp_recurring_codes($id) {
	qpp_change_form_update();
	if( isset( $_POST['Submit'])) {
        $reccuring['couponnumber'] = stripslashes($_POST['couponnumber']);
        $reccuring['couponget'] = stripslashes($_POST['couponget']);
        $reccuring['duplicate'] = stripslashes($_POST['duplicate']);
        $reccuring= array('code','coupontype','couponpercent','couponfixed');
        if ($coupon['couponnumber'] < 1) $coupon['couponnumber'] = 1;
        for ($i=1; $i<=$coupon['couponnumber']; $i++) {
            foreach ( $options as $item) $coupon[$item.$i] = stripslashes($_POST[$item.$i]);
            if (!$coupon['coupontype'.$i]) $coupon['coupontype'.$i] = 'percent'.$i;
	...
	$qpp_setup = qpp_get_stored_setup();
    $id = $qpp_setup['current'];
    $recurring = qpp_get_stored_recurring($id);
    print_r($recurring);
    $content ='<div class="qpp-settings"><div class="qpp-options">';
	if ($id) $content .='<h2>Reccuring Payment Options for ' . $id . '</h2>';
	else $content .='<h2>Recurring Payment Options</h2>';
	$content .= qpp_change_form($qpp_setup);
	$content .= '<form method="post" action="">
		<p<span<b>Note:</b> Leave fields blank if you don\'t want to use them</span></p>
    <p><input type="submit" name="Submit" class="button-primary" style="color: #FFF;" value="Save Changes" /> <input type="submit" name="Reset" class="button-primary" style="color: #FFF;" value="Reset" onclick="return window.confirm( \'Are you sure you want to reset the coupon codes?\' );"/></p>
