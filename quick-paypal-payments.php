...
function qpp_verify_form(&$formvalues,&$errors,$form) {
    ...
    if ($qpp['allow_amount']) $errors['amount'] = '';
...
function qpp_display_form( $values, $errors, $id ) {
    ...
    <span id="rep_place'.$id.'"><h2>'.$send['waiting'].'</h2></span>
    </div>
    <form id="frmPayment" name="frmPayment" method="post" action="">';
		...
		switch ( $name ) {
            ...
            if (!$values['setref']) {
                ...
                if ($values['explode']) {
                    ...
                    $content .= '<p class="input" >'.$values['reference'].'</p>
                    <input type="hidden" name="setref" value="' . $values['setref'] . '" />';
                }
            }
            break;	
            case 'field2':
            ...
            case 'field3':
            if ($qpp['use_quantity']) {
                $content .= '<p><span class="input">'.$qpp['quantitylabel'].'</span>
                <input type="text" style=" '.$errors['quantity'].'width:3em;margin-left:5px" label="quantity" name="quantity" value="' . $values['quantity'] . '" onfocus="qppclear(this, \'' . $values['quantity'] . '\')" onblur="qpprecall(this, \'' . $values['quantity'] . '\')"/>';
            ...
            case 'field4':
            if (!$values['setpay']) {
                $content .= '<p><input type="text" '.$errors['amount'].'label="Amount" name="amount" value="' . $values['amount'] . '" onfocus="qppclear(this, \'' . $values['amount'] . '\')" onblur="qpprecall(this, \'' . $values['amount'] . '\')"/></p>';
            } else {
                if ($values['explodepay']) {
                    $checked = 'checked';$ref = explode(",",$values['amount']);
                    $content .= '<p class="payment" >'.$qpp['shortcodeamount'].'<br>';
                    foreach ($ref as $item) {
                        $content .=  '<label><input type="radio" style="margin:0; padding: 0; border:none;width:auto;" name="amount" value="' .  $item . '" ' . $checked . '> ' .  $item . '</label><br>';
                        $checked='';
                    }
                    $content .= '</p>';
                } else {
                    $content .= '<p class="input" >' . $values['amount'] . '</p><input type="hidden" name="amount" value="'.$values['amount'].'" />';
                }
            }
            break;
            case 'field5':
            ...
            <p><input type="submit" value="'.$qpp['couponbutton'].'" id="couponsubmit" name="qppapply'.$id.'" /></p>';}
            ...
            case 'field12':
            if ($qpp['userecurring']) {
                $content .= '<p>' . $qpp['recurringblurb']. ' ' . $qpp['recurringhowmany'].' ' . $qpp['recurringperiod'] . '</p>';
                $checked = 'checked';$ref = explode(",",$values['recurring']);
            }
            break;
        }
    }
...
function qpp_process_form($values,$id) {
    ...
    echo $qpp['recurringhowmany'].'/'.$qpp['recurringperiod'];
	...
	$content = '<h2>'.$send['waiting'].'</h2>
        ...
        if ($qpp['userecurring']) $content .= '<input type="hidden" name="cmd" value="_xclick-subscriptions">';
        else $content .= '<input type="hidden" name="cmd" value="_cart">'; 
	...
	if ($qpp['userecurring']) $content .= '<input type="hidden" name="a1" value="' . $check . '">
    <input type="hidden" name="p1" value="' .$qpp['recurringhowmany'] . '"><input type="hidden" name="t1" value="' .$qpp['recurring'] . '>';
...
function qpp_loop($atts) {
    ...
    if (isset($_POST['qppapply'.$form]) || isset($_POST['qppsubmit'.$form]) || isset($_POST['qppsubmit'.$form.'_x'])) {
        ...
        $arr = array('reference','amount','stock','quantity','option1','couponblurb','maths','thesum','answer','termschecked');
...
function qpp_generate_css() {
	...
	foreach ($arr as $item) {
		...
		if ($style['font'] == 'plugin') {
            $font = "font-family: ".$style['text-font-family'].";
            font-size: ".$style['text-font-size'].";
            color: ".$style['text-font-colour'].";
            line-height:100%;";
			$inputfont = "font-family: ".$style['font-family'].";
            font-size: ".$style['font-size'].";
            color: ".$style['font-colour'].";";
...
function qpp_get_stored_options ($id) {
    ...
    if (!strpos($qpp['sort'],'12')) {$qpp['sort'] = $qpp['sort'].',field11,field12';$update = 'update';}
...
function qpp_get_default_options () {
	$qpp = array();
	$qpp['sort'] = 'field1,field4,field2,field3,field5,field6,field7,field9,field8,field10,field11,field12';
    ...
    $qpp['userecurring'] = '';
    $qpp['recurringblurb'] = 'Subscription for';
    $qpp['recurring'] = 'M';
